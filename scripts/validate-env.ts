...
class EnvironmentValidator {
  ...
  // 验证S3存储桶配置
  private async validateS3Configuration(): Promise<ValidationResult> {
    console.log('\n🔍 验证S3存储桶配置...');
    ...
    try {
      ...
      return {
        success: true,
        message: 'S3存储桶配置正确',
        details: { fileCount: files.length, files: files.slice(0, 5) } // 只显示前5个文件
      };
    } catch (error) {
  ...
  // 验证Supabase认证服务连接
  private async validateAuthConnection(): Promise<ValidationResult> {
    console.log('\n🔍 验证Supabase认证服务连接...');
  ...
  // 测试安全文件上传
  private async testSecureUpload(): Promise<ValidationResult> {
    console.log('\n🔍 测试安全文件上传...');
  ...
  // 测试安全文件删除
  private async testSecureDelete(): Promise<ValidationResult> {
    console.log('\n🔍 测试安全文件删除...');
    ...
    try {
      ...
      // 获取文件列表
      const files = await listCloudFiles();
      const testFiles = files.filter((f: any) => f.key.startsWith('test/'));
